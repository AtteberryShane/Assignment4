...
EXTEND_POSE = [0.0, -0.9, 0.6, 0.3]
PLACE_POSE = [0.0, -0.6, 0.45, 0.15]
...
class BottleAutonomyController(Node):
    def __init__(self):
        ...
        self.get_logger().info("Bottle autonomy controller started")
    ...
    def yolo_callback(self, msg):
        ...
        bottle = self.find_best_bottle(data.get("detections", []))
        if bottle is None:
            return
    ...
    def run_search(self):
        ...
        self.publish_base_cmd(0.0, SEARCH_TURN_SPEED)
    ...
    def send_arm_goal(self, positions, duration_sec):
        if not self.arm_action_client.server_is_ready():
            self.get_logger().warning("Arm action server not available")
            return
